...
constexpr std::array<TerminalDef, 1> TerminalNameList{
    TerminalDef{util::plot::GnuTerminal::wxt, "wxt noenhanced",
                "wxWidgets Terminal"},
};
...
void GnuPlot::MakeScript(std::ostream &script) const {
  ...
  script << "set terminal " << TerminalToScriptText(terminal_) << " size "
         << x_size_ << "," << y_size_ << std::endl;
  ...
  if (y2_column < nof_cols) {
    script << "set y2label \"" << data_file_.Unit(y2_column) << "\""
           << std::endl;
  }
...
void GnuPlot::Show() {
  try {
    ...
    // Need to restore the current directory otherwise is the temporary
