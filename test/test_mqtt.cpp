...
TEST(Mqtt, Mqtt3Client) { // NOLINT
  auto listen = UtilFactory::CreateListen("ListenConsole", "LISMQTT");
  listen->SetActive(true);
  listen->SetLogLevel(3);
  listen->Start();

  auto client = UtilFactory::CreatePubSubClient(PubSubType::Mqtt3Client);
