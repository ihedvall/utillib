 ...
 *
 * The class is used to configure the default logger for an application. There is basically 2 default
 * logger to choose between. For large applications, it's recommended to log messages onto a file while
 * simpler applications typically those without a GUI, may log all message onto the cout stream.
 *
 * Note that you may add many loggers to the application. Normally is the default logger good enough but more
 * advanced system may add more logger interfaces.
 *
...
/**
 * The log configuration is a singleton object that defines the default logger as well
 * as holding the actual list of loggers a.k.a. log chain.
 ...
 */
  ILogger *GetLogger(const std::string &logger_name) const; ///< Returns a logger by name.

  /**
 * Return the log file if any exist.
 * @param [in] logger_name Name of the logger.
 * @return Returns the full path to its log file or empty string if no one exist.
 */
  std::string GetLogFile(const std::string& logger_name = "Default") const; ///< Returns a log file.
 ...
 */
  ...
  [[nodiscard]] bool Enabled() const; ///< Returns true if the loggers are enabled.

  void RootDir(const std::string &root_dir); ///< Sets the root directory.
  [[nodiscard]] const std::string &RootDir() const; ///< Return the root directory.
