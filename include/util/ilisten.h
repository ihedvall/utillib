 ...
 *
 * The IListen class defines an interface against various listen implementation
 * objects. The class hides the implementation classes.
 */
class IListen {
 public:
   ...
   *
   * Function used when parsing byte buffer typically in protocol applications.
   * By default it converts the byte buffer to a hexadecimal string.
   * @param ns1970 Time stamp nanoseconds since 1970.
   * @param pre_text Pre-text string.
 ...
 protected:
   ...
   *
   * All text lines generates a listen message that is sent to a TCP/IP server
   * or via a proxy server (shared memory).
